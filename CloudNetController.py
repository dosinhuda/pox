...
class CloudNetController (EventMixin):
    ...
    def install_end_to_end_IP_path(self, event, dst_dpid, final_port, packet):
        ...
        if dst_dpid == event.dpid:
            ...
            self.switches[event.dpid].install_output_flow_rule(final_port, match, 10)
        else:
            paths_list = self.switches[event.dpid]._paths[dst_dpid]
            selected_path = random.choice(paths_list)

            node_list = selected_path[::-1] #reverse selected path nodes
            log.info("New path")
            pp(node_list[::-1])
            for i, dpid in enumerate(node_list):
                #print "i:", i, "dpid:", dpid
                if i == 0: #destination dpid
                     self.switches[dpid].install_output_flow_rule(final_port, match, 10)
                else:
                    if i == (len(node_list)-1): # packet out from the source so as not to lose the current packets
                        self.switches[dpid].send_packet(self.sw_sw_ports[(dpid, node_list[i-1])], packet)
                    self.switches[dpid].install_output_flow_rule(self.sw_sw_ports[(dpid,node_list[i-1])], match, 10)

    def install_migrated_end_to_end_IP_path(self, event, dst_dpid, dst_port, packet, forward_path=True):
        match_normal_headers = of.ofp_match()
        match_normal_headers.dl_type = 0x0800
        match_normal_headers.nw_src = packet.next.srcip
        match_normal_headers.nw_dst = packet.next.dstip
        match_new_headers = None

        if dst_dpid == event.dpid:
            if forward_path:
                new_IP = self.old_migrated_IPs[IPAddr(packet.next.dstip)]
                dst_mac = self.arpmap[new_IP][0]
                self.switches[event.dpid].send_forward_migrated_packet(dst_port, dst_mac, new_IP, packet)
                self.switches[event.dpid].install_forward_migration_rule(dst_port, dst_mac, new_IP, match_normal_headers, 10)
            else:
                old_IP = self.new_migrated_IPs[IPAddr(packet.next.srcip)]
                src_mac = self.arpmap[old_IP][0]
                self.switches[event.dpid].send_reverse_migrated_packet(dst_port, src_mac, old_IP, packet)
                self.switches[dpid].install_reverse_migration_rule(dst_port, src_mac, old_IP, match_normal_headers, 10)
            ...
            if forward_path:
                new_IP = self.old_migrated_IPs[IPAddr(packet.next.dstip)]
                match_new_headers = of.ofp_match()
                match_new_headers.dl_type = 0x0800
                match_new_headers.nw_src = packet.next.srcip
                match_new_headers.nw_dst = new_IP
            else:
                old_IP = self.new_migrated_IPs[IPAddr(packet.next.srcip)]
                match_new_headers = of.ofp_match()
                match_new_headers.dl_type = 0x0800
                match_new_headers.nw_src = old_IP
                match_new_headers.nw_dst = packet.next.dstip

            node_list = selected_path[::-1] #reverse selected path nodes for installation
            if forward_path:
                log.info("New forward path")
            else:
                log.info("New reverse path")
            pp(node_list[::-1])
            for i, dpid in enumerate(node_list):
                #print "i:", i, "dpid:", dpid
                if i == 0: #Destination dpid
                    self.switches[dpid].install_output_flow_rule(dst_port, match_new_headers, 10)
                elif i == (len(node_list)-1): # PacketIn dpid
                    if forward_path:
                        new_IP = self.old_migrated_IPs[IPAddr(packet.next.dstip)]
                        dst_mac = self.arpmap[new_IP][0]
                        self.switches[event.dpid].send_forward_migrated_packet(self.sw_sw_ports[(dpid,node_list[i-1])], dst_mac, new_IP, packet)
                        self.switches[dpid].install_forward_migration_rule(self.sw_sw_ports[(dpid,node_list[i-1])],
                                                                           dst_mac, new_IP, match_normal_headers, 10)
                    else:
                        old_IP = self.new_migrated_IPs[IPAddr(packet.next.srcip)]
                        src_mac = self.arpmap[old_IP][0]
                        self.switches[event.dpid].send_reverse_migrated_packet(self.sw_sw_ports[(dpid,node_list[i-1])], src_mac, old_IP, packet)                        
                        self.switches[dpid].install_reverse_migration_rule(self.sw_sw_ports[(dpid,node_list[i-1])],
                                                                           src_mac, old_IP, match_normal_headers, 10)
                else: # intermediary dpid
                    self.switches[dpid].install_output_flow_rule(self.sw_sw_ports[(dpid,node_list[i-1])], match_new_headers, 10)
...
class SwitchWithPaths (EventMixin):
    ...
    def flood_on_switch_edge(self, packet, no_flood_ports):
        for port in self.ports:
            if (not (port.port_no in no_flood_ports)) and (port.port_no <= MAX_PHYS_PORTS):
    ...
    def send_arp_reply(self, packet, dst_port, req_mac):
        ...
        self.send_packet(dst_port, ether)
...
def ShortestPaths(switches, adjs):
    for source in switches:
        switches[source].clearPaths()
        topo_graph = nx.from_dict_of_lists(adjs)
        print "============================="
        for dst in switches:
            if (source==dst):
                continue
            print "SOURCE:", source, "DEST:", dst
            try:
                nx_paths = nx.all_shortest_paths(topo_graph,source,dst)
                all_shortest_paths = [p for p in nx_paths]
            except nx.NetworkXNoPath:
                return False
            print all_shortest_paths
            switches[source].appendPaths(dst, all_shortest_paths)
            print "============================="
    return True
