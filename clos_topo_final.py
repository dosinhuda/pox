...
class ClosTopo(Topo):
    ...
    def __init__(self, fanout, cores, **opts):
        ...
        for c in irange(1, cores):
            self.addSwitch('cor%s' %c)
            for a in irange(1, cores*fanout):
                self.addSwitch('agg%s' %(a+cores))
                self.addLink('agg%s' %(a+cores), 'cor%s' %c)
        ...
        for a in irange(1, cores*fanout):
            for e in irange(1, cores*(fanout**2)):
                self.addSwitch('edg%s' %(e+counter))
                self.addLink('edg%s' %(e+counter), 'agg%s' %(a+cores))
        ...
        for e in irange(1, cores*(fanout**2)):
            for h in irange(1, fanout):
                self.addHost('h%s' %(host_pod_offset+h))
                self.addLink('edg%s' %(e+counter), 'h%s' %(host_pod_offset+h))
